...
class LockManager {
 ...
 private:
  ...
  auto DFS(const txn_id_t &txn_id, std::unordered_set<txn_id_t> &visisted, std::vector<txn_id_t> &path, txn_id_t *txn)
      -> bool {
    visisted.insert(txn_id);
    path.push_back(txn_id);
    auto it = waits_for_.find(txn_id);
    if (it != waits_for_.end()) {
      for (auto item : it->second) {
        if (std::find(path.begin(), path.end(), item) != path.end()) {
          for (auto cycle = std::find(path.begin(), path.end(), item); cycle != path.end(); cycle++) {
            *txn = std::max(*txn, *cycle);
          }
          return true;
        }
        if (visisted.find(item) == visisted.end()) {
          if (DFS(item, visisted, path, txn)) {
            return true;
          }
        }
      }
  ...
  auto RemoveTableLockFromTxn(Transaction *txn, LockMode lock_mode, const table_oid_t &oid) -> void {
    ...
    switch (lock_mode) {
      case LockMode::SHARED:
        erased = txn->GetSharedTableLockSet()->erase(oid);
        break;
      case LockMode::EXCLUSIVE:
        erased = txn->GetExclusiveTableLockSet()->erase(oid);
        break;
      case LockMode::INTENTION_EXCLUSIVE:
        erased = txn->GetIntentionExclusiveTableLockSet()->erase(oid);
        break;
      case LockMode::INTENTION_SHARED:
        erased = txn->GetIntentionSharedTableLockSet()->erase(oid);
        break;
      case LockMode::SHARED_INTENTION_EXCLUSIVE:
        erased = txn->GetSharedIntentionExclusiveTableLockSet()->erase(oid);
        break;
    }
    BUSTUB_ASSERT(erased == 1, "Lock not found in transaction's lock set");
    txn->UnlockTxn();
  }
  ...
  auto AreLocksCompatible(LockMode first, LockMode second) -> bool {
    ...
    if (first == LockMode::INTENTION_EXCLUSIVE) {
      return second == LockMode::INTENTION_SHARED || second == LockMode::INTENTION_EXCLUSIVE;
    }
    if (first == LockMode::SHARED) {
      return second == LockMode::SHARED || second == LockMode::INTENTION_SHARED;
    }
    if (first == LockMode::SHARED_INTENTION_EXCLUSIVE) {
      return second == LockMode::INTENTION_SHARED;
    }
    if (first == LockMode::EXCLUSIVE) {
      return false;
    }
    return false;
  }

  void AddTableLockToTxn(Transaction *txn, LockMode mode, table_oid_t oid) {
    txn->LockTxn();
    switch (mode) {
      case LockMode::SHARED:
